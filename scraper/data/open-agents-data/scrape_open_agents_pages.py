"""
Scrapes individual project pages for Open Agents projects.
Reads from data/open-agents-data/projects_raw.json
Writes to data/open-agents-data/projects_full.json

Usage:
  python scrape_open_agents_pages.py
  python scrape_open_agents_pages.py --test     # scrape first project only
  python scrape_open_agents_pages.py --limit=10
"""
...
INPUT_FILE  = Path(__file__).parent / "data" / "open-agents-data" / "projects_raw.json"
OUTPUT_FILE = Path(__file__).parent / "data" / "open-agents-data" / "projects_full.json"
...
def main():
    ...
    if test_mode:
        projects = projects[:1]
        print(f"TEST MODE — scraping 1 project\n")
    elif limit:
